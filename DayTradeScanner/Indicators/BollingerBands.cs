...
namespace DayTradeScanner
{
	...
	{
		public BollingerBands(List<MarketCandle> candles, int candle, int length = 20, int deviations = 2)
		{
			var prices = candles.Skip(candle).Select(e => e.ClosePrice).Take(length).ToList();
